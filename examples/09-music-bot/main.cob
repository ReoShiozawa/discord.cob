       ...
       PROGRAM-ID. EXAMPLE-MUSIC-BOT.
       *> JP: music bot の最小起動導線をまとめた example です。
       *> JP: command bootstrap -> login -> bot tick loop までを現行 API で一通り並べます。
       *> EN: Example that assembles the minimal startup path for the music bot.
       *> EN: It shows command bootstrap -> login -> bot tick loop using the
       *> EN: current high-level APIs.
